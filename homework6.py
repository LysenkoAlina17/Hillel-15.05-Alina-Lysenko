...
def get_tiket_price (age):
    if age < 7:
        return(f"Тобі ж {age}! Де твої батьки?")
    elif age < 16:
        return(f"Тобі лише {age}, а це е фільм для дорослих!")
    elif age > 65:
        return(f"Вам {age}? Покажіть пенсійне посвідчення!")
    elif "7" in str(age):
        return(f"Вам {age}, вам пощастить")
    else:
        return(f"Незважаючи на те, що вам {age}, білетів всеодно нема!")
